...
GMAX = 1.24 # maximal growth rate for bacteria [gC/(gC day)], KEYLINK was 1.24
...
for param in (paramsToTestNames):
    for paramChange in (paramChanges):  
        ...
        for i in range(len(DOMinput_treatments)):
            numruns = numruns + 1
            # I want to use the values from the dict, for sensitivity, so i put all of the values back in the variable (not the fastest way)
...
# initializing variables (what changes during run)
 ...
 #                   print('mainLine270 DOM, bact, fungi, MAOMs, MAOMp', DOM,bact, fungi, MAOMs, MAOMp)
    ...
    # MAOM formation
                ...
                if CN_DOM > 0: 
                    DOM, DOM_N, CN_DOM, DOM_sub, MAOMp, MAOMp_sub, MAOMs, MAOMs_sub, CN_MAOMs = mf.calcMAOM(MicrobialC, DOM_N, CN_DOM, fractionSA, MAOMp, MAOMp_sub, maxMAOMp, DOM, DOM_sub, MAOMs, MAOMs_sub, maxMAOMs, MAOMsmaxrate, MAOMpmaxrate, MM_DOM_MAOM, maxEffectBactMAOM, MM_Bact_MAOM, maxEffectN_MAOM, MM_N_MAOM, maxEffectSA_MAOM, MM_SA_MAOM, CN_MAOMp, CN_MAOMs)
