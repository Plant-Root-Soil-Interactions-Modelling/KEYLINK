...
def calc_sim_likelyhood(simulation, measurement, error):  # calculates loglikelihood
    if pd.isna(
        measurement
    ...
    # if err == 0:
    #     print("Sim_loglikelihood:", sim_loglikelihood)
    #     print(type(sim_loglikelihood))

    #     print("Error:", error)
    #     print(type(error))

    #     print("log:", np.log(error))
    #     print(type(np.log(error)))

    # print("division by zero:", ((measurement - simulation) / error))
    # print(type((measurement - simulation) / error))
...
def read_parameter_data(inputCalibParamfile):
    ...
    maximum_option_all = parameterForCalibList[
        "MaxParam"
    ]  # input is always in same filename
    CalibParamInit = parameterForCalibList[
        "InitParam"
    ]  # MaximumOptionDic = json.load(inputfileParamMax)
    minimal_option_all = parameterForCalibList[
        "MinParam"
    ]  # input is always in same filename
...
def find_new_parameters(
    calibrated_parameters_values,
    variance_parameter_space,
    minimal_option,
    maximum_option,
    keys,
    Allparam,
):
    candidate_value = ra.multivariate_normal(
        calibrated_parameters_values, variance_parameter_space
    )  # new parameter value
    for j in range(len(calibrated_parameters_values)):  # For each parameter
        ref_min = min(
            0, candidate_value[j] - minimal_option[j]
        )  # reflection from minimum if you walked too far
        ref_max = max(
            0, candidate_value[j] - maximum_option[j]
        )  # reflection from maximum
        if ref_min != 0 or ref_max != 0:
            ...
            candidate_value[j] = (
                candidate_value[j] - 2 * ref_min - 2 * ref_max
            )  # new values
            ...
            if ref_min != 0 or ref_max != 0:
                candidate_value[j] = random.gauss(
                    calibrated_parameters_values[j], variance_parameter_space[j, j]
                )
    ...
    candidate_parameters = dict(
        zip(keys, candidate_value)
    )  # Combine the new values to the parameters name
...
def split_alphanumeric_list(
    string_list,
):  # returns nonnumeric and numeric part of string list as two lists
    non_numeric_list = []
    numeric_list = []
