    ...
    for i in range(int(std), int(std+tStop)):
        ...
        for j in range (NrGroups+1): #--here
            if usingNewTemp == 0:
                ...
                modt[j], loggedTemps, T_OPT[j], T_MIN[j], T_MAX[j], absT_MIN[j], absT_MAX[j], sensor[j], meanSensor[j]  = mf.calcmodt(loggedTemps, temp, T_OPT[j], T_MIN[j], T_MAX[j], absT_MIN[j], absT_MAX[j], sensor[j], meanSensor[j], j, acclimSpeed[j])
        ...
        if optimisation is True:# -- Olivier
            if allOptimise==1:
                for orgGroup in range(NrGroups):
                    ...
                    selectedParamVal = mf.OptimiseParamater(argNum, orgGroup, i, mf.fCompSpecies, B, avail, modt, litterCN, SOMCN, GMAX, mf, CN, MCN, MREC, pH, recLit, FAEC, pfaec, rRESP, KS, DEATH, CtoMyc, temp, T_OPT, T_MIN, T_MAX, loggedTemps, absT_MIN, absT_MAX, sensor, meanSensor, usingNewTemp, acclimSpeed)
                    if argNum == 2:
                ...
                orgGroup = 1 #set this to fungi for now for testing
                argNum = 1 #sets the argument to change. Currently 2 = GMAX and anything else is modt 
                selectedParamVal = mf.OptimiseParamater(argNum, orgGroup, i, mf.fCompSpecies, B, avail, modt, litterCN, SOMCN, GMAX, mf, CN, MCN, MREC, pH, recLit, FAEC, pfaec, rRESP, KS, DEATH, CtoMyc, temp, T_OPT, T_MIN, T_MAX, loggedTemps, absT_MIN, absT_MAX, sensor, meanSensor, usingNewTemp, acclimSpeed)
                if argNum == 2:
                    GMAX[orgGroup] = selectedParamVal #update the current GMAX value to the newly optimised one
