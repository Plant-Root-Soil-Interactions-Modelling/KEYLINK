...
#output dataframe list
    ...
    GMAX=AllParam['GMAX'] #maximal growth rate for bacteria [gC/(gC day)], KEYLINK was 1.24
    GMAXfungi=AllParam['GMAXfungi']  #maximal growth rate for fungi [gC/(gC day)], KEYLINK
    ...
    pCN=AllParam['pCN'] #sensitivity to CN ratio of consumed substrate, values 0-1, taken from KEYLINK (value for bacteria)
    recMAOM= AllParam['recMAOM'] #recalcitrance of MAOM, (recalcitrance of POM assumed 0)
    RESPbact=AllParam['RESPbact'] #respiration rate of bacteria, [gC/(gC day)], was 0.05 KEYLINK 
    RESPfungi=AllParam['RESPfungi'] #respiration rate of fungi, [gC/(gC day)], was 0.03 KEYLINK 
    ...
    DOMinput=treatmentVar['DOMinput'] #10 #DOM added in each addition [gC/m3], Jílková2022
    CN_DOMinput=treatmentVar['CN_DOMinput'] #  #CN of the daily input [unitless], Jílková2022: leachates 80, exudates 6
    ...
    # treatment, d, resp, resp_sub, POM, MAOM, bact_total, fungi, POM_sub, MAOM_sub, bact_total_sub, fungi_sub]
        column_names=['treatment',	'day', 'resp','resp_sub', 
                      'POM', 'MAOM', 'bact_total', 'fungi','POM_sub', 'MAOM_sub', 'bact_total_sub', 'fungi_sub']
        results_df = pd.DataFrame(columns=column_names)
    else: #normal runs
      # numruns=1
      numParams=1
    ...
    # initializing variables (what changes during run)
       ...
       # microbial growth on DOM and priming, only susing MAOMs
                    ...
                    AllC = DOM + POM + MAOM + bact_total + fungi + resp - DOMadded
                    print('line313', treatment, d, AllC)
                    print(DOM,POM,MAOM,bact_total,fungi,resp,DOMadded)
                    if CN_DOM>0: 
     ...
     #                   print('mainLine270 DOM, bact, fungi, MAOMs, MAOMp', DOM,bact, fungi, MAOMs, MAOMp)
                        ...
                        respPriming = 0
                        respPriming_sub = 0
                    print('line 324 main DOM+respDOM+bact_DOM=', DOM+respDOM+bact_DOM) 
     ...
     #               if (bact<0):
      #                  print('mainLine290 DOM, bact, fungi', DOM,bact, fungi)
           ...
           #add up things
                    ...
                    resp_sub = baselineRespBact_sub*(baselineRespBact/resp) + baselineRespFungi_sub*(baselineRespFungi/resp) + respDOM_sub*(respDOM/resp) + respPriming_sub*(respPriming/resp)
         ...
         ############# Plotting   #############       
                if(Plotting): #transform data for Plotting to adjusted units matching the data
                    ...
                    def Dailyplot2(outBact_DOM_sub, outBact_sub, outFungi_sub, outDOM_sub,outPOM_sub, outMAOMs_sub,  outMAOMp_sub):
                        ...
                        p2.plot(time_d, outMAOM_sub, label="MAOM")
